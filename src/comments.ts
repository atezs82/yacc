...
const QUOTE_MAX = 80;
const ellipsize = (s: string): string => s.length > QUOTE_MAX ? s.slice(0, QUOTE_MAX) + '…' : s;
...
function placePopover(
  ...
  const side: 'above' | 'below' = opts.sideHint ??
    (refRect.top - H - GAP < 8 ? 'below' : 'above');
...
class SelPopover {
  ...
  contains(el: Element): boolean { return !!el.closest('#sel-popover'); }
  ...
  private reposition(): void {
    if (this.refRect) placePopover(this.root, this.refRect, { sideHint: this.side });
  }
  ...
  private appendTurnEl(role: Role, text: string): HTMLElement {
    if (this.threadEl.style.display === 'none') this.threadEl.style.display = '';
  ...
  private submit(): void {
    ...
    const turns = trailing ? [...this.turns, { role: 'user' as const, text: trailing }] : this.turns.slice();
  ...
  private buildConversationPayload(newUserText: string): string {
    const parts = this.turns.map(t => (t.role === 'user' ? 'User: ' : 'Assistant: ') + t.text);
...
class ViewPopover {
  ...
  contains(el: Element): boolean { return !!el.closest('#view-popover'); }

  open(anchor: HTMLButtonElement, comment: Comment, id: string, isLatest: boolean): void {
    this.viewId = id;
    this.exitEdit();
  ...
  private enterEdit(): void {
    ...
    const editable = userTurns.length > 0 ? userTurns[userTurns.length - 1]!.text : '';
  ...
  private onSave(): void {
    ...
    if (c) {
      for (let i = c.turns.length - 1; i >= 0; i--) {
        if (c.turns[i]!.role === 'user') { c.turns[i]!.text = newText; break; }
      }
...
document.addEventListener('mousedown', e => {
  ...
  if (!viewPop.contains(target) && !target.closest('.comment-anchor')) viewPop.close();
});
