...
function testBin(bin: string): Promise<string> {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error('timed out after 5 s')), 5000);
...
function readBody(req: IncomingMessage): Promise<Record<string, unknown> | null> {
  ...
  return new Promise(resolve => {
    ...
    req.on('data', (c: Buffer) => {
      raw += c;
      if (raw.length > MAX) { req.destroy(); resolve(null); }
    });
    req.on('end',  () => { try { resolve(JSON.parse(raw) as Record<string, unknown>); } catch { resolve({}); } });
...
function streamCLI(prompt: string, files: FileAttachment[], res: ServerResponse): void {
  ...
  for (const f of files) {
    try {
      ...
      if (isPrintable(text)) {
        ...
        textParts.push(`[Attached file: ${f.name} — binary content, not shown]`);
      }
    } catch {
  ...
  const effectivePrompt = textParts.length > 0
    ? textParts.join('\n\n') + '\n\n' + prompt
    : prompt;
  ...
  proc.on('close', (code) => {
    streaming = false;
    if (!sentResult) res.write(sse({ error: `Claude exited without a response (code ${code ?? '?'})` }));
    res.end();
  });
}
...
const server = http.createServer(async (req: IncomingMessage, res: ServerResponse) => {
  ...
  if (method === 'GET' && pathname === '/api/status') {
    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({ bin: CLAUDE_BIN, version: CLAUDE_VERSION }));
  }
